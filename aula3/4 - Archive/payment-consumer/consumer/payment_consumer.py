...
from consumer.checkout_client import CheckoutClient
...
class PaymentConsumer:
    def __init__(self):
        ...
        self.checkout_client = CheckoutClient(
            httpx.AsyncClient(base_url=self.checkout_url)
        )
    ...
    async def process_message(self, message: aio_pika.IncomingMessage):
        ...
        try:
            ...
            result = await self.payment_client.process(
            ...
            if result["error"]:
                ...
                result = await self.checkout_client.payment_processed(
                    checkout_id=payment_request.checkout_id,
                    payment_id=result["transaction_id"],
                )
                if result["error"]:
                    print(f"Failed to notify checkout service: {result['error']}")
